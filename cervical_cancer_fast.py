...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=50, device='cuda'):
    ...
    print(f"\n开始快速训练，共 {num_epochs} 轮...")
    print("优化: 更轻量模型 + 更大batch + 简化增强 + 50轮...")
...
def evaluate_model(model, test_loader, device='cuda'):
    checkpoint = torch.load('best_cervical_model_fast.pth')
    ...
    print("\n【各类别详细指标】")
    print(f"{'类别':<20} {'精确率':<15} {'召回率':<15} {'F1分数':<10} {'样本数':<10}")
    print("-" * 100)
    ...
    print("\n混淆矩阵已保存为 confusion_matrix_fast.png")
...
def plot_training_history(train_losses, val_losses, train_accs, val_accs):
    ...
    plt.tight_layout()
    plt.savefig('training_history_fast.png', dpi=300, bbox_inches='tight')
    print("训练历史曲线已保存为 training_history_fast.png")
...
def main():
    ...
    train_transform = transforms.Compose([
        transforms.Resize((224, 224)),  # 直接 resize 到目标尺寸，不需要先 256 再 crop
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.RandomRotation(15),  # 减少旋转角度
        transforms.ColorJitter(brightness=0.1, contrast=0.1),  # 更简单的颜色变换
        transforms.ToTensor(),
    ...
    # WeightedRandomSampler 但不过采样（保持原始样本数）
    train_labels_np = np.array(train_labels)
    class_sample_counts = np.array([np.sum(train_labels_np == i) for i in range(7)])
    weights = 1. / torch.tensor(class_sample_counts, dtype=torch.float)
    weights[CLASS_NAMES['severe_dysplastic']] *= 1.5
    samples_weights = weights[train_labels_np]
    sampler = WeightedRandomSampler(weights=samples_weights, num_samples=len(samples_weights), replacement=True)  # 不×2！
    ...
    model, train_losses, val_losses, train_accs, val_accs = train_model(
        model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=50, device=device)
    ...
    print("\n" + "=" * 100)
    print("【快速训练版本】任务完成！预计时间从数小时降至 30-60 分钟")
    print("=" * 100)
